...
fn remove_nth(xs: &[u32], n: usize) -> Vec<u32> {
    let mut ys =
        Vec::with_capacity(xs.len().saturating_sub(1));
    ys.extend_from_slice(&xs[..n]);
    ys.extend_from_slice(&xs[n + 1..]);
    ys
}
...
fn is_good_enough(xs: &[u32]) -> bool {
    let ys: Vec<Vec<u32>> = xs
        .iter()
        .enumerate()
        .map(|(i, _)| remove_nth(xs, i))
        .collect();

    ys.iter().any(|xs| is_safe(xs))
}
